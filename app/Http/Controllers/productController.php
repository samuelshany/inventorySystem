...
class productController extends Controller
{
    ...
    function sellProductForm($id)
    {
       $mess='';
        $product= products::where('id',$id)->get();
        $clients = Clients::get();
        return view('product/sellProduct',compact('product','clients','mess'));
    }
    function sellProductFormE($id,$mess)
    {
    ...
    function sellProduct($id,Request $request)
    {
        ...
        { $mess="enter lower amount";
            return $this->sellProductFormE($id,$mess);
        }
        else
        {
    ...
    {
        $name=$request->name;
        $products=products::where('productName','like',$name.'%')->get();
